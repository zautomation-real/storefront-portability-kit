import { escapeHtml, formatMoney, presentationLayout, productBodyHtml, productZoomMode } from "./lib.mjs";
...
export function renderProductPreview(brand, product, catalog = { products: [] }) {
  ...
  const alternateMedia = variants.map((variant) => variant.media).find((media) => media.image !== initialMedia.image);
  const alternateMediaMarkup = alternateMedia ? `<div class="media product-media-gallery__alternate" aria-label="Alternate view of ${escapeHtml(product.name)}"><img src="${rootPath}${escapeHtml(alternateMedia.image)}" alt="${escapeHtml(alternateMedia.alt)}" width="1600" height="1000" loading="lazy" draggable="false" data-preview-product-alternate-image></div>` : "";
  ...
  const content = `<main id="main"><section class="preview-product">
    <div class="product-media-gallery${alternateMedia ? " product-media-gallery--extended" : ""}"><div class="media preview-product__media product-zoom" data-product-zoom data-product-zoom-mode="${zoomMode}" tabindex="0" role="button" aria-pressed="false" aria-label="Zoom image for ${escapeHtml(product.name)}" data-zoom-label="Zoom image for ${escapeHtml(product.name)}" data-unzoom-label="Exit image zoom for ${escapeHtml(product.name)}"><img src="${rootPath}${escapeHtml(initialMedia.image)}" alt="${escapeHtml(initialMedia.alt)}" width="1200" height="1400" loading="eager" fetchpriority="high" draggable="false" data-preview-product-image></div>${alternateMediaMarkup}</div>
